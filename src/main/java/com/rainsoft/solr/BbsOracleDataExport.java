...
public class BbsOracleDataExport extends BaseOracleDataExport {
    ...
    public static void exportOracleByTime(Date curTime, boolean isImportHBase) {
        ...
        //Oracle查询参数：开始时间
        String curTimeParam = DateFormatUtils.DATE_TIME_FORMAT.format(curTime);

        //从导入记录中获取最后导入的日期
        String lastExportRecord = recordMap.get(recordKey);

        Date lastExportTime = null;
        try {
            lastExportTime = DateUtils.parseDate(lastExportRecord, "yyyy-MM-dd HH:mm:ss");
         ...
         */
        Date tmpDate = DateUtils.addDays(lastExportTime, 10);
        if (tmpDate.before(curTime)) {
            curTime = tmpDate;
        }

        logger.info("{} : 开始索引 {} 到 {} 的数据", task, curTimeParam, lastExportRecord);
        //获取数据库指定捕获时间段的数据
        List<String[]> dataList = dao.getBbsByHours(lastExportRecord, curTimeParam);
...
//                oracleContentTableDataExportSolr(javaRDD, task);
                ...
                logger.info(
                        ...
                        curTime.getTime(),
                        lastExportTime.getTime()
                );


                //导入记录写入Map
                recordMap.put(recordKey, curTimeParam);
            ...
            logger.info("Oracle数据库 {} 表在{} 至 {} 时间段内没有数据", "reg_content_bbs", curTimeParam, lastExportRecord);
        }
    }
