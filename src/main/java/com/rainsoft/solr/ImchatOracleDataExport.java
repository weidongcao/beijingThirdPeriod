...
public class ImchatOracleDataExport extends BaseOracleDataExport {
    ...
    //imChatDao
    private static FtpDao imChatDao = (FtpDao) context.getBean("imchatDao");

    //字段名
    private static String[] columns = FieldConstants.COLUMN_MAP.get("oracle_reg_content_im_chat");

    public static void imchatExportOracleByHours(Date startTime, JavaSparkContext sc) {
        ...
        if (!BaseOracleDataExport.SUCCESS_STATUS.equals(BaseOracleDataExport.recordMap.get(imChatRecord))) {
            ...
            //获取数据库指定捕获时间段的数据
            List<String[]> dataList = imChatDao.getFtpByHours(startTimeParam, endTimeParam);
            logger.info("从数据库查询数据结束");

            //导入Solr
            boolean flat =oracleContentTableDataExportSolr(dataList, contentType);
            //导入HBase
//            imChatExportHBase(dataList, sc);
     ...
     */
    public static void imChatExportHBase(List<String[]> list, JavaSparkContext sc) {
        ...
        logger.info("Oracle {} 数据写入HBase完成,数据量: {}", "reg_content_im_chat", list.size());
        list.clear();
    }
