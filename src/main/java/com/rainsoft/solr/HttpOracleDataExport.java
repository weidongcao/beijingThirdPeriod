...
public class HttpOracleDataExport extends BaseOracleDataExport {
    ...
    //httpDao
    private static FtpDao httpDao = (FtpDao) context.getBean("httpDao");

    //字段名
    private static String[] columns = FieldConstants.COLUMN_MAP.get("oracle_reg_content_http");

    public static void httpExportOracleByHours(Date startTime, JavaSparkContext sc) {
        ...
        if (!BaseOracleDataExport.SUCCESS_STATUS.equals(BaseOracleDataExport.recordMap.get(httpRecord))) {
            ...
            //获取数据库指定捕获时间段的数据
            List<String[]> dataList = httpDao.getFtpByHours(startTimeParam, endTimeParam);
            logger.info("从数据库查询数据结束");

            //导入Solr
            boolean flat =oracleContentTableDataExportSolr(dataList, contentType);
            //导入HBase
            httpExportHBase(dataList, sc);
     ...
     */
    public static void httpExportHBase(List<String[]> list, JavaSparkContext sc) {
        ...
        logger.info("Oracle {} 数据写入HBase完成,数据量: {}", "reg_content_http", list.size());
        list.clear();
    }
